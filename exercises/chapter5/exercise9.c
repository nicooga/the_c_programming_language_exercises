...
int invoke_error(char function_name[]);
...
int main(void)
{
    ...
    else return invoke_error(function_name);
...
int day_of_year(unsigned short year, char month, char day)
{
    ...
    for (i = 1; i < month; i++) {
        char *selected_daytab = *(DAYTABS + leap);
        day += *(selected_daytab + i);
    }
...
)
{
    ...
    for (i = 1; year_day > DAYTABS[leap][i]; i++) {
        char *selected_daytab = *(DAYTABS + leap);
        year_day -= *(selected_daytab + i);
    }
